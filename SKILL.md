...
description: Present proposals, demos, and previews to the user visually through HTML instead of describing them in text. ALWAYS use this skill, before drafting a text answer, when either (1) the user asks to see, show, demo, preview, compare, or visually confirm anything — trigger phrases include "show me", "let me see", "demo", "preview", "how does it look", "which looks better", — or (2) Claude itself is about to propose or explain something whose merit must be judged by eye: UI designs, screen layouts, mockups, wireframes, document structures, diagrams, page compositions, styling and theming options, or any A/B alternatives, even if the user never mentions HTML or design. If a draft response would describe visual qualities in words (colors, spacing, arrangement, hierarchy, look and feel), stop — that is the signal to use this skill and generate a single proposal HTML with the embedded sticky-note feedback layer instead. Exception for proportionality — when the visual matter is clearly minor (a single-property tweak, a small color/wording adjustment, a change the user has already decided), do not auto-generate the HTML; instead answer normally and offer the choice in one short sentence, e.g. "Want me to show this in HTML so you can review it visually?", generating only if accepted. Also consult this skill whenever the user pastes a feedback JSON (a JSON object containing "issue" and "notes" keys) so it can be interpreted correctly.
---
